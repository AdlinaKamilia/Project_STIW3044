...
<body>
    ...
    <div class="section">
        <form method="post">
            <div style = "padding-top: 10px ; padding-bottom:10px ; justify-content: center;display: flex;">
                <pre><label for="search">Search parcel: </label> <input type="text" id="search" name="search" placeholder="Enter Parcel Tracking Number"> <label for="date">Date: </label> <input type="date" name="date"> <label for="status">Status: </label> <select id="status" name="status"><option value="" <?php echo ($status == '') ? 'selected' : ''; ?>>Default</option><option value="Collected" style="color: green;" <?php echo ($status == 'Collected') ? 'selected' : ''; ?> >Collected</option><option value="In Hub" style="color: red;" <?php echo ($status == 'In Hub') ? 'selected' : ''; ?>>In Hub</option></select> <label for="status">Payment: </label><select id="payment" name="payment"><option value="" <?php echo ($payment == '') ? 'selected' : ''; ?>>Default</option><option value="Paid" <?php echo ($payment == 'Paid') ? 'selected' : ''; ?> style="color: green;">Paid</option><option value="Outstanding" <?php echo ($payment == 'Outstanding') ? 'selected' : ''; ?> style="color: red;">Outstanding</option></select> <input type="submit" value="Filter" style="background-color: #7b1034;color: white;padding-left: 20px;padding-right: 20px;border-radius: 40px; border-style: none; font-weight: bold;"> </pre>  
            </div>
        </form>
        <table>
            <thead>
            ...
            <tbody>
                <?php
                    for($i=0; $i<count($parcelList); $i++){
                        ...
                        echo "<td>" . $parcelList[$i]['parcel_payment'] . "</td>";
                        echo "<td>" . $parcelList[$i]['parcel_status'] . "</td>";
                        echo '<td>
                        <div class="action-container">
                            <button class="click-container" style="background-color: #252F9C;" onClick= "goToDetail('. $parcelList[$i]['parcel_id'] .',' . $staff_id .')";>
                                <div>
                            ...
                            <button class="click-container" style=" background-color: #7b1034;">
                                <div>
                            ...
                            <button class="click-container" style="background-color: #df0101;" onClick= "confirmDelete('. $parcelList[$i]['parcel_id'] .',' . $staff_id .')";>
                                <div>
    ...
    <footer style="position: fixed;">
        <div class="add-parcel-button">
            <a href = "addparcel.php?staff_id=1">
